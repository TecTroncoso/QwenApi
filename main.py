...
if not UPSTASH_REDIS_URL:
    raise RuntimeError("UPSTASH_REDIS_URL debe estar definida.")
...
def process_cookies_and_extract_token(b64_string: str | None):
    ...
    if not b64_string:
        print("[WARN] Cookies no definidas. Usando Auth Token de respaldo.")
    ...
    try:
        ...
        print(f"[ERROR CRÍTICO] Procesando cookies: {e}.")
        QWEN_AUTH_TOKEN, QWEN_COOKIE_STRING = QWEN_AUTH_TOKEN_FALLBACK or "", ""

process_cookies_and_extract_token(QWEN_COOKIES_JSON_B64)

QWEN_HEADERS = {
    "Accept": "application/json", "Content-Type": "application/json; charset=UTF-8",
    "Authorization": QWEN_AUTH_TOKEN, "Cookie": QWEN_COOKIE_STRING, "Origin": "https://chat.qwen.ai", "Referer": "https://chat.qwen.ai/",
...
async def stream_qwen_to_openai_format(
    ...
    try:
        async with client.stream("POST", url, json=payload, headers=headers) as response:
            ...
            async for line in response.aiter_lines():
                ...
                try:
                    ...
                    if is_answer_phase and "message_id" in qwen_chunk and final_answer_message_id is None:
                        final_answer_message_id = qwen_chunk["message_id"]

                    if requested_model == MODEL_QWEN_FINAL and not is_answer_phase: continue
        ...
        if final_answer_message_id:
            state.last_parent_id = final_answer_message_id
            save_conversation_state(qwen_chat_id, state)
            print(f"✅ Contexto actualizado para la conversación {qwen_chat_id} con parent_id: {final_answer_message_id[:8]}...")
        else:
...
async def chat_completions_endpoint(request: OpenAIChatCompletionRequest, client: httpx.AsyncClient = Depends(get_http_client), x_conversation_id: str | None = Header(None, alias="X-Conversation-ID")):
    ...
    generator = stream_qwen_to_openai_format(client=client, qwen_chat_id=qwen_chat_id, state=state, message=last_message, requested_model=request.model)
